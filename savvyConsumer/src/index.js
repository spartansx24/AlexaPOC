...
function getWelcomeResponse(response) {
    ...
    var repromptText = "Please choose a category by saying, books <break time=\"0.2s\" />"
    ...
    response.ask({speech: speechOutput, type: AlexaSkill.speechOutput.PLAIN_TEXT},
            {speech: "<speak>" + repromptText + "</speak>", type: AlexaSkill.speechOutput.SSML});
}
...
function getTopSellers(intent, session, response) {
    ...
    if (lookupCategory) {
        ...
        repromptText = "I'm not sure what the category is, you can say, "
            + "books <break time=\"0.2s\" /> fashion <break time=\"0.2s\" /> movie <break time=\"0.2s\" /> kitchen.";
        response.ask({speech: speechOutput, type: AlexaSkill.speechOutput.PLAIN_TEXT},
                {speech: "<speak>" + repromptText + "</speak>", type: AlexaSkill.speechOutput.SSML});
    }
}
    ...
    if (sessionAttributes[KEY_CURRENT_INDEX]) {
        ...
        for (var i = 0; i < PAGINATION_SIZE; i++) {
            if (sessionAttributes[current]) {
                ...
                if (current < MAX_ITEMS - 1) {
                    speechOutput = speechOutput + "<say-as interpret-as=\"ordinal\">" + numberInList + "</say-as>. "
                        + sessionAttributes[current] + ". ";
                } else {
                    speechOutput = speechOutput + "And the <say-as interpret-as=\"ordinal\">" + numberInList + "</say-as> top seller is. "
        ...
        if (current < MAX_ITEMS) {
            ...
            response.ask({speech: "<speak>" + speechOutput + "</speak>", type: AlexaSkill.speechOutput.SSML},
                    {speech: "<speak> Would you like to hear more top sellers? Please say yes or no. </speak>", type: AlexaSkill.speechOutput.SSML});
        } else {
...
function helpTheUser(intent, session, response) {
    ...
    var repromptText = "I'm sorry I didn't understand that. You can say things like, books <break time=\"0.2s\" /> movies <break time=\"0.2s\" /> music. " +
